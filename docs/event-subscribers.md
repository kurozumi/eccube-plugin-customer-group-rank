...
| 2 | 0 | 本体 `LoginHistoryListener` | ログイン履歴（管理者のみ） |
| 3 | 0 | 本体 `SecurityListener` | **保存されていたカートを取り込んで購入フローを流し直す** |

**先に当てないと困るのは3番のため。** 会員グループは価格・販売可否・配送・支払方法を
...
にあり、**先頭かどうか**を見ている。priority を外すと落ちる。
